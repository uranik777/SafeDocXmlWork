...
public class FileThread  extends Thread implements SDLoggable{
	...
	public FileThread( ) throws IOException {
	...
	public synchronized void run() {
		...
		this.setTerminal("");
		this.setUserId("");		
		SDLogger.getApplicationLog().add(this,"File thread started","ok");
		while(m_NeedExit==false){
			while(m_Queue.size()>0){
				this.setSignature("");
				this.setTerminal("");
				this.setUserId("");				
				SDLogger.getApplicationLog().add(this,"File queue size: "+m_Queue.size(),"");
				com._4ds.safedoc.action.File l_File=new com._4ds.safedoc.action.File();
				try {
					...
					SDLogger.getApplicationLog().add(this,"Get File object from queue signature:"+l_XmlObject.getSignature(), l_XmlObject.getM_ObjectFileName() );
					l_File.execute( l_XmlObject );
					java.io.File l_File1=new java.io.File( l_XmlObject.getM_ObjectFileName() );
					SDLogger.getApplicationLog().add(this,"Remove File object from queue signature:"+l_XmlObject.getSignature(), l_XmlObject.getM_ObjectFileName() );
					l_File1.delete();
					m_Queue.remove(0);
				} catch (Exception e) {
					SDLogger.getApplicationLog().add(this,"File thread error:",e.getMessage());
					e.printStackTrace();
					m_Queue.remove(0);
				}
			}
		...
		SDLogger.getApplicationLog().add(this,"File thread end","ok");
	}
	...
	private String getFileName() throws IOException {
		...
		do{
	        ...
	        l_FileName= SafeDocServlet.get_config().getQueuePath()+l_timestring+"-FileObject.xml";
