...
public class PrintHTML implements Printable {
	...
	public void print(byte[] x_byte) throws IOException, BadLocationException {
		InputStream lInputStream = new java.io.ByteArrayInputStream(x_byte);

		HTMLDocument lHTMLDocument;
		HTMLEditorKit lHTMLEditorKit = new HTMLEditorKit();
		lHTMLDocument = (HTMLDocument) lHTMLEditorKit.createDefaultDocument();
		lHTMLDocument.putProperty("IgnoreCharsetDirective", Boolean.TRUE);
		lHTMLEditorKit.read(lInputStream,lHTMLDocument,0);
		lInputStream.close();
		

		setDocument(lHTMLDocument);
		printDialog();
	}
